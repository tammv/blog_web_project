  ...
  useEffect(() => {
    const fetchLatestPosts = async () => {
      try {
        ...
        setLatestPosts(response.data.posts); // Assuming the response contains posts array under 'posts' key
      } catch (error) {
        console.error("Error fetching latest posts:", error);
        setError("Error fetching latest posts. Please try again later."); // Set error state
      } finally {
      ...
      <section className="py-10 bg-gray-50 sm:py-16 lg:py-24 dark:bg-gray-800">
        <div className="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
          <div className="flex items-end justify-between">
            ...
            <div className="hidden lg:flex lg:items-center lg:space-x-3">{/* Navigation buttons if needed */}</div>
          </div>
                ...
                <div key={post._id} className="overflow-hidden bg-white rounded-md shadow dark:bg-gray-900">
                  <div className="p-8 xl:p-10">
                    ...
                    <p className="mt-4 text-base text-gray-600 dark:text-gray-300">
                      {post.content.slice(0, 15)}... {/* Display excerpt or summary */}
                    </p>
